 ...
 */
export function JsTypedAssignmentSlideView({ slide, storageKey, breadcrumb, slideJumpDots, onPass }: Props) {
  const { codePx, prosePx } = useSlideFontSize();
  const template = t(slide.template);
  ...
  const codePanel = (
    <div className="flex-1 flex flex-col rounded-2xl overflow-hidden min-h-0
      ...
      <div className="flex flex-wrap gap-2 p-3 border-t border-stone-200 dark:border-white/[0.08]">
        <button
          onClick={runChecks}
        ...
        <button
          ...
          className="px-4 py-2 min-h-[44px] sm:min-h-0 sm:px-3 sm:py-1.5 rounded-lg text-sm
                     ...
                     dark:bg-[#2c303a] dark:hover:bg-[#252934] dark:active:bg-[#1b1e26] dark:text-white"
        >
          {t(ui.reset)}
        </button>
        {hasLegend && (
        ...
        <div className="mx-4 sm:mx-10 mb-4" style={{ fontSize: `${prosePx}px` }}>
          {allPass ? (
            ...
            <div className="rounded-xl px-4 py-3
                            ...
                            dark:bg-[#1f232c] dark:text-indigo-200 dark:border-white/[0.08]">
              {t(slide.goalHint)}
            </div>
          ) : null}
        </div>
  ...
  return (
    <input
      ...
      className="inline-block align-baseline mx-0.5 px-1.5 py-0 rounded
                 ...
                 dark:bg-indigo-500/20 dark:text-stone-100 dark:ring-indigo-400/60
...
function SceneMount({
  ...
  switch (allegory.kind) {
    case "door":
      return <DoorScene config={allegory.config} run={run} replayKey={replayKey} hideConditionLabel />;
    case "fork":
      return <ForkScene config={allegory.config} run={run} replayKey={replayKey} hideConditionLabel />;
    case "conveyor":
    ...
    case "crosswalk":
      return <CrosswalkAllegory config={allegory.config} run={run} replayKey={replayKey} hideConditionLabel />;
    case "loop-result":
